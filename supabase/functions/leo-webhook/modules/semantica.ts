...
export function inferirTipoClienteTexto(texto: string): "porta_instalada" | "revenda" | null {
  const t = norm(texto);
...
export function inferirMedidasTexto(texto: string): { largura: number; altura: number } | null {
...
export function inferirAdicionaisTexto(texto: string, permitirNegacaoGenerica = false): { portinhola: boolean; alcapao: boolean } | null {
  const t = norm(texto);
...
export function inferirEntregaTexto(texto: string): { quer_entrega: boolean } | null {
  const t = norm(texto);
  if (!t.trim()) return null;
...
export function inferirSubtipoRevendaTexto(texto: string): "kit" | "pecas" | null {
  const t = norm(texto);
  if (!t.trim()) return null;
...
export function inferirRespostaPortinholaCorte(textoNormalizado: string): boolean | undefined {
